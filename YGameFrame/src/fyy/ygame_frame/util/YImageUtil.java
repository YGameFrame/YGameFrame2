...
public final class YImageUtil
{
	...
	public static Bitmap[] getBitmapArray(Resources res, int[] iImgIds)
	{
		...
		for (int i = 0; i < iImgIds.length; i++)
			btmp_arr[i] = BitmapFactory.decodeResource(res, iImgIds[i]);
	 ...
	 * @return 被拉伸后的位图数组
	 */
	public static Bitmap[] stretchImageArray(Bitmap bitmaps[], int iSideLength)
	{
		Bitmap[] btmp_arrStretched = new Bitmap[bitmaps.length];

		for (int i = 0; i < bitmaps.length; i++)
		{
			btmp_arrStretched[i] = Bitmap.createScaledBitmap(bitmaps[i],
					iSideLength, iSideLength, true);
	 ...
	 * @return 拆分后的位图对象数组
	 */
	public static Bitmap[] splitImage(int iRowSum, int iColumnSum, Bitmap btmpSrc)
	{
		...
		for (int i = 0; i < iRowSum; i++)
			for (int j = 0; j < iColumnSum; j++)
				btmp_arr[i * iColumnSum + j] = Bitmap
						.createBitmap(btmpSrc, j * iSideLength, i
								* iSideLength,
								iSideLength, iSideLength);
	...
	public static void recycleBitmapArray(Bitmap[]... bitmaps)
	{
		for (int i = 0; i < bitmaps.length; i++)
			for (int j = 0; j < bitmaps[i].length; j++)
				if (null != bitmaps[i][j] && !bitmaps[i][j].isRecycled())
					bitmaps[i][j].recycle();
	}
}
